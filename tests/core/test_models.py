...
from safaribooks.core import models
...
class TestBookInfo:
    def test_creation_with_valid_data(self):
        book_info = models.BookInfo(
            ...
            authors=[models.Author(name="Guido van Rossum")],
            publishers=[models.Publisher(name="O'Reilly")],
            subjects=[models.Subject(name="Python"), models.Subject(name="Programming")],
            issued="2024-01-01",
        )
        assert book_info.title == "Python Cookbook"
        assert book_info.isbn == "9781234567890"
        assert len(book_info.authors) == 1
        assert book_info.authors[0].name == "Guido van Rossum"
        assert len(book_info.subjects) == 2
    ...
    def test_creation_with_optional_defaults(self):
        ...
        assert book_info.cover is None
        assert book_info.issued is None
...
class TestChapter:
    def test_creation_with_stylesheets(self):
        ch = models.Chapter(
            ...
            images=["img/fig1.png"],
            stylesheets=[models.Stylesheet(url="style.css")],
...
class TestTocEntry:
    ...
    def test_deeply_nested_children(self):
        ...
        nested_child = parent.children[0].children[0]
        assert nested_child.label == "Subsection"
...
class TestCookieSet:
    def test_valid_cookies_pass(self):
        ...
        cs = models.CookieSet(cookies=cookies)
    ...
    def test_missing_required_cookies_raises(self):
        with pytest.raises(ValidationError, match="Missing required cookies"):
            models.CookieSet(cookies={"jwt": "tok"})
...
class TestSearchResult:
    def test_book_id_prefers_archive_id(self):
        search_result = models.SearchResult(
            title="Test", archive_id="12345", isbn="67890", identifier="abc"
        )
        assert search_result.book_id == "12345"

    def test_book_id_falls_back_to_isbn(self):
        search_result = models.SearchResult(title="Test", isbn="67890", identifier="abc")
        assert search_result.book_id == "67890"

    def test_book_id_falls_back_to_identifier(self):
        search_result = models.SearchResult(title="Test", identifier="abc")
        assert search_result.book_id == "abc"

    def test_book_id_empty_when_no_ids(self):
        search_result = models.SearchResult(title="Test")
        assert search_result.book_id == ""

    def test_creation_with_defaults(self):
        search_result = models.SearchResult(title="Minimal")
        assert search_result.isbn == ""
        assert search_result.authors == []
        assert search_result.publishers == ""
        assert search_result.issued == ""
    ...
    def test_creation_with_all_fields(self):
        ...
        assert search_result.title == "Full Book"
        assert len(search_result.authors) == 2
...
class TestSearchResponse:
    def test_creation_with_results(self):
        resp = models.SearchResponse(
            results=[
                models.SearchResult(title="Book 1"),
                models.SearchResult(title="Book 2"),
            ],
            count=2,
            total=10,
        )
